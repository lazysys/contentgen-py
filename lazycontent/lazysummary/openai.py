...
class OpenAISummarizer(Summarizer):
	...
	# FIXME: if into is not recognized we silently fail, this is quite hard to debug in Nodezator for example
	# FIXME: as LazyCommon might be imported twice differently (such with Nodezator), we only check name not actual inheritance. Breaks original plan.
	def summarize(self, entries: List[Entry], into: Type[Content]) -> List[str]:
		...
		if into.__name__ == Microblog.__name__:
				...
				while True:
					if len(answer) > 280:
						...
						return [answer]
		elif into.__name__ == Thread.__name__:
			...
			return answer.split("\n---\n")
		elif into.__name__ == Article.__name__:
		...
		elif into.__name__ == ShortVideo.__name__:
			...
			return self._convert_answer_to_std(answer)
		elif into.__name__ == LongVideo.__name__:
			...
			return self._convert_answer_to_std(answer)
		elif into.__name__ == Image.__name__:
		...
		elif into.__name__ == Carousel.__name__:
